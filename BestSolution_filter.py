...
def ob_sus_correct(sim_tolerate_num, ssim_datas,
                   select_pro, select_lable, train_pro, y_train, noise_threshold):
    '''
    ...
    total = np.concatenate([ssim_topk_values, ssim_topk_indices], axis=-1)

    _, ssim_unique_indexs = np.unique(total, return_index=True, axis=0)
    ...
    # Line是select的指针
    for line_number in tqdm(ssim_unique_indexs):
