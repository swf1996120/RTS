...
def get_model_output_and_feature(
    ...
    second_last_layers = keras.models.Model(model.input, model.layers[-2].output)


    features = second_last_layers.predict(batch_x)
...
def get_ensemble_model_output_and_feature(
    ...
    if ensemble_method == 'hard':
        num_classes = None
    for model in models:
        batch_output, batch_feature = get_model_output_and_feature(
            model,
            batch_x,
        )
        batch_feature_list.append(batch_feature)
        if ensemble_method == 'hard':
            ...
            raise ValueError(f'Not supported ensemble method: {ensemble_method}!')
    # Concatenates the features of the models in the ensemble.
    concat_batch_feature = np.concatenate(batch_feature_list, axis=1)
