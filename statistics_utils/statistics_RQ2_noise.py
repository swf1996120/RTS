...
def merge_all(mop_name_arr, sample_num, merge_csv_name_list):
    # cov, div,  all
    # cov2, div2, all2
    # cov3, div3, all3
...
def get_df_merge_all(merge_csv_name_list):
    df_merge_all = None
...
def get_RQ2_tables(res_path, merge_csv_name_list):
    for b in [True, False]:
        get_RQ2_tables_detail(res_path, use_p_value=b, merge_csv_name_list=merge_csv_name_list)
...
def get_RQ2_tables_detail(res_path, use_p_value=True, merge_csv_name_list=None):
    if not use_p_value:
    ...
    df_merge = get_df_merge_all(
        merge_csv_name_list=merge_csv_name_list)  # df_merge: result/all,  result/all2.csv, 所有结果合并
...
def RQ2_noise():
    ...
    mop_name_arr_ori = ["ORI"]
    mop_name_arr_rest = ["RG", "RP", "IR", "CR"]
    merge_csv_name_ori_list = ["all_ori", "all2_ori", "all3_ori"]
    merge_csv_name_noise_list = ["all_noise", "all2_noise", "all3_noise"]

    merge_all(mop_name_arr_ori, sample_num=1, merge_csv_name_list=merge_csv_name_ori_list)
    merge_all(mop_name_arr_rest, sample_num=1, merge_csv_name_list=merge_csv_name_noise_list)

    # tab
    RQ2_table_path_ori = get_RQ2_table_path() + "_ori" + ".csv"
    RQ2_table_path_rest = get_RQ2_table_path() + "_rest" + ".csv"

    get_RQ2_tables(RQ2_table_path_ori, merge_csv_name_list=merge_csv_name_ori_list)
    get_RQ2_tables(RQ2_table_path_rest,merge_csv_name_list=merge_csv_name_noise_list)
