...
from statistics_utils.statistics_utils import get_exp_name, get_base_fig_dir, get_base_table_dir, get_order_arr, \
    get_RQ1_table_path, get_raw_result_dir, num_to_str, get_RQ1_1_table_path
...
def get_error_num(max_size, error_dir, rank_name, i=0, mop_name_arr=None):
...
def statistics(csv_path, ratio, mop_name_arr):
    arr1 = ["DeepDAC", "DeepDiv", "Gini", "CES", "LSA", "MAX_P", "Random", "Kmeas_sample",
            "KCenterGreedy", "var_rank", "ood_gini", "msp_gini", "entrory_dropout_rank", "BALDDropout", "BADGE_sample",
            "CLUE_sample"]
    arr2 = ["NAC", "NBC", "SNAC", "TKNC"]
    sp_num = 0
    rank_name_list = get_order_arr()  # all rank methods
    ...
    for data_name, v_arr in tqdm(model_conf.model_data.items()):
        ...
        for model_name in v_arr:
            print(model_name, data_name, mop_name_arr)
            ...
            for rank_name in rank_name_list:
                ...
                error_num = get_error_num(max_size, error_dir, rank_name, i=sp_num, mop_name_arr=mop_name_arr)
                # 保留三位小数
...
def RQ1_1():
    ...
    for type in types:
        if type == "ori":
            ...
            mop_name_arr = ["RG", "RP", "IR", "CR"]
        for ratio in [0.025, 0.05, 0.075, 0.1]:
            csv_path = get_RQ1_1_table_path() + "{}_{}.csv".format(ratio, type)
            statistics(csv_path, ratio, mop_name_arr)
