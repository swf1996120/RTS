...
def retrain_detail(temp_model_path, x_s, y_s, X_train, Y_train, x_val_dict, y_val_dict, model_path, nb_classes,
                   verbose=1, only_add=False, data_name=None, seed=0):
    ...
    trained_model = retrain_model(temp_model_path, ori_model, Xa_train, Ya_train_vec, X_val, Y_val_vec, idx=seed,
                                  verbose=verbose, data_name=data_name)
    eee = time.time()
...
def retrain_model(temp_path, ori_model, x_si, y_si_vector, Xa_test, Ya_test_vec, idx=0, verbose=1, data_name=None):
...
def train_model(model, filepath, X_train, Y_train, X_test, Y_test, epochs=15, verbose=1, data_name=None):
    if data_name in [model_conf.mnist, model_conf.fashion]:
        batch_size = 128
    else:
        batch_size = 64
