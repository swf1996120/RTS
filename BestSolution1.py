...
def obtain_ssim(data_name, sim_tolerate_num, x_select, x_train, y_train, x_select_hash, x_train_hash):
    ...
    for line_number, single_select in tqdm(enumerate(select_tensor)):  # 测试用例的原始序列
        ...
        for label, indexs in class_matrix.items():
            img2 = x_train[np.random.choice(indexs, 50, replace=False)]
...
def ob_sus_correct(sim_tolerate_num, ssim_datas,
    ...
    ssim_1_values, ssim_1_indices = ssim_topk_values[:, 0], ssim_topk_indices[:, 0]
...
def construct_empty_array(internal_number, nb_classes, wise=2):
    # 需要构造一个
    from itertools import combinations
    coverage_array = {}
    parameter_com = list(combinations(range(0, nb_classes), wise))
    for single in parameter_com:
        coverage_array[single] = np.zeros([internal_number] * wise)
...
def get_coverage(test, coverage_array: typing.Mapping, nb_classes, wise=2):
    count = None
    from itertools import combinations
    parameter_com = list(combinations(range(0, nb_classes), wise))
    # pair 类似于 category
    for pair in parameter_com:
        para_coverage = coverage_array.get(pair, None)  #
        assert para_coverage.all() != None
        # 需要转置
        index = np.transpose(test[:, pair]).tolist()
        # 取出tests 在改组合下的bit并与之前的拼接
        count = concat_df(count, para_coverage[tuple(index)].reshape(-1, 1))
    statis_coverage = len(parameter_com) - np.count_nonzero(count, axis=-1)
    return statis_coverage


def update_coverage(test, coverage_array: typing.Mapping, nb_classes, wise=2):
    from itertools import combinations
    parameter_com = list(combinations(range(0, nb_classes), wise))
    for pair in parameter_com:
        para_coverage = coverage_array.get(pair, None)  # 取出对应的覆盖矩阵
        assert para_coverage.all() != None
        index = test[list(pair)]
        if isinstance(para_coverage[0], float):
            para_coverage[index] = 1
        else:
            para_coverage[tuple(index)] = 1


def rank(candate_lists, internal_number, nb_classes, wise=2, select_size=4000):
    '''
    candidate_lists: array_index,  uncertain , index of candidate test sets,
    '''
    total_length = len(candate_lists)
    rank = []
    coverage = construct_empty_array(internal_number, nb_classes, wise=wise)

    terminal = False

    # array_index, index of whole sequence, uncertain
    for _ in tqdm(range(select_size)):
        # statis_coverage = np.apply_along_axis(get_coverage, -1, candate_lists[:, :-2].astype(np.int),
        #                                       coverage_array=coverage, nb_classes=nb_classes, wise=wise)
        statis_coverage = get_coverage(candate_lists[:, :-2].astype(np.int), coverage_array=coverage,
                                       nb_classes=nb_classes, wise=wise)
        ...
        update_coverage(candate_lists[best_index][:-2].astype(np.int), coverage_array=coverage,
                        nb_classes=nb_classes, wise=wise)  # 更新
        rank.append(candate_lists[best_index][-1])

        if len(rank) == select_size:
            terminal = True
...
def conquer(select_lable, select_pro, indexs, number_internals, nb_classes, wise=2, select_size=4000):
    ...
    # 每个test对应的的索引位置;uncertains:不确定度； indexs：在这个候选集中的原始位置,
    ...
    scores, terminal = rank(candidate_lists, number_internals, nb_classes=nb_classes, wise=wise,
